const {
    ...
    getTxInfo,
    getTransactions,
    getStakingInfo} = require('../dist/src');
...
describe('Cosmos API tests', () => {
  it('Transfers money to account', async () => {
      ...
      assert.strictEqual(exptectedBalance.toString(), newBalance.result[0].amount);
  }).timeout(100000);
  ...
  assert.strictEqual(exptectedBalance.toString(), newBalance.result[0].balance);
  }).timeout(100000);
